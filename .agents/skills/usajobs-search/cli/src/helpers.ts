...
export async function apiGet(path: string): Promise<UsaJobsResponse | null> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    let rawBody: string
    try {
      ;({ status, body: rawBody } = await httpGet(url, headers))
    } catch (e) {
      throw new Error(
    ...
    if (status === 401 || status === 403) {
      throw new MissingCredentialsError(
        `USAJOBS API rejected the credentials (${status}). Check USAJOBS_API_KEY and USAJOBS_EMAIL ` +
          "(the email must match the one you registered at https://developer.usajobs.gov/apirequest/).",
      )
    }
    if (status === 429 || status >= 500) {
      if (attempt === maxRetries) {
        throw new Error(`USAJOBS API request failed: ${status}`)
      }
    ...
    let body: UsaJobsResponse | null
    try {
      body = JSON.parse(rawBody) as UsaJobsResponse
    } catch {
      body = null
    }
